...
MODULE_PATH = "../tables.mbt"
"""The path of the emitted MoonBit module (relative to the working directory)"""
...
def load_solidus_transparent(
    ...
    for cp in ccc_above_1:
        if cp not in [0xFE00, 0xFE0F]:
            assert cjk_width_map[cp].table_width() != CharWidthInTable.SPECIAL, (
                f"U+{cp:X}"
            )

    # Remove Arabic combining marks (0x64B to 0x65F) from solidus transparent
    for cp in range(0x64B, 0x65F + 1):
        ccc_above_1.discard(cp)
...
def load_text_presentation_sequences() -> list[Codepoint]:
    ...
    for cp in text_presentation_seq_codepoints.intersection(default_emoji_codepoints):
        ...
        if cp not in range(0x1F200, 0x1F300):
            codepoints.append(cp)
...
{cfg}fn lookup_width{cjk_lo}(c : Char) -> (UInt, WidthInfo) {{
    let cp = c.to_int()

    let t1_offset = width_root{cjk_cap.lower()}[cp >> {TABLE_SPLITS[1]}]
    ...
    let t2_offset = width_middle[t1_offset.reinterpret_as_int()][(cp >> {TABLE_SPLITS[0]}) & 0x{(2 ** (TABLE_SPLITS[1] - TABLE_SPLITS[0]) - 1):X}]
    ...
    let packed_widths = width_leaves[t2_offset.reinterpret_as_int()][(cp >> 2) & 0x{(2 ** (TABLE_SPLITS[0] - 2) - 1):X}]
...
"""
    ...
    for (lo, hi), width in special_ranges:
        if hi == lo:
            ...
            # Use MoonBit range patterns
            s += f"            '\\u{{{lo:X}}}'..='\\u{{{hi:X}}}'"
        if width.is_carried():
            # Convert to snake_case constant name
            width_info = width.name.lower() + "_width_info"
            s += f" => ({width.width_alone()}, {width_info})\n"
        else:
            s += f" => ({width.width_alone()}, default_width_info)\n"

    s += f"""            _ => (2, emoji_presentation_width_info)
        }}
    }}
}}
...
{cfg}fn char_width{cjk_lo}_impl(c : Char) -> Int? {{
    if c < '\\u{{7F}}' {{
        if c >= '\\u{{20}}' {{
        ...
        Some(lookup_width{cjk_lo}(c).0.reinterpret_as_int())
    }} else {{
...
///| Ambiguous width characters are treated as {ambig}.
{cfg}fn width_in_str{cjk_lo}(c : Char, next_info : WidthInfo) -> (Int, WidthInfo) {{
    let mut next_info = next_info

    // Special handling for Arabic combining marks to preserve ligature state
    if next_info == joining_group_alef_width_info &&
      c >= '\\u{{64B}}' &&
      c <= '\\u{{65F}}' {{
      return (0, joining_group_alef_width_info)
    }}

    // Special handling for ZWJ emoji sequences
    if next_info == zwj_emoji_presentation_width_info &&
      lookup_width{cjk_lo}(c).1 == emoji_presentation_width_info {{
      return (0, emoji_presentation_width_info)
    }}
    if next_info.is_emoji_presentation() && !next_info.is_zwj_emoji_presentation() {{
      if starts_emoji_presentation_seq(c) {{
        return (2, emoji_presentation_width_info)
      }} else {{
    ...
    if is_cjk:
        s += """
    if (next_info == combining_long_solidus_overlay_width_info || next_info == solidus_overlay_alef_width_info) && (c == '<' || c == '=' || c == '>') {
        return (2, default_width_info)
    }"""

    s += """
    if c <= '\\u{A0}' {
        match c {
            '\\n' => (1, line_feed_width_info)
            '\\r' => if next_info == line_feed_width_info { (0, default_width_info) } else { (1, default_width_info) }
            _ => (1, default_width_info)
        }
    } else {
        // Fast path
        if next_info != default_width_info {
            if c == '\\u{FE0F}' {
    ...
    s += """if next_info.is_vs1_2() {
                if c == '\\u{2018}' || c == '\\u{2019}' || c == '\\u{201C}' || c == '\\u{201D}' {
                    return ("""

    s += str(2 - is_cjk)

    s += """, default_width_info)
                } else {
            ...
            if next_info.is_ligature_transparent() {
                ...
                } else if is_char_ligature_transparent(c) {
                    return (0, next_info)
                }
            }

            // Use if-guards instead of pattern matching for width state constants
            """
    if is_cjk:
        s += """
            if next_info == combining_long_solidus_overlay_width_info && is_solidus_transparent(c) {
                return (
                    lookup_width_cjk(c).0.reinterpret_as_int(),
                    combining_long_solidus_overlay_width_info,
                )
            }
            if next_info == joining_group_alef_width_info && c == '\\u{0338}' {
                return (0, solidus_overlay_alef_width_info)
            }
        ...
        s += """
            // Arabic Lam-Alef ligature
            if next_info == joining_group_alef_width_info && c == '\\u{338}' {
                return (0, solidus_overlay_alef_width_info)
            }
            if (
                next_info == joining_group_alef_width_info ||
                next_info == solidus_overlay_alef_width_info
              ) && ("""
    ...
    for lo, hi in joining_group_lam:
        ...
        if hi == lo:
            ...
            s += f"(c >= '\\u{{{lo:X}}}' && c <= '\\u{{{hi:X}}}')"
    s += """) {
                return (0, default_width_info)
            }"""
    if is_cjk:
        s += """
            if next_info == solidus_overlay_alef_width_info && ("""
        # Add the same character patterns for SolidusOverlayAlef
        tail = False
        for lo, hi in joining_group_lam:
            if tail:
                s += " || "
    ...
    s += """
            if next_info == joining_group_alef_width_info && is_transparent_zero_width(c) {
                return (0, joining_group_alef_width_info)
            }

            // Hebrew Alef-ZWJ-Lamed ligature
            if next_info == zwj_hebrew_letter_lamed_width_info && c == '\\u{05D0}' {
                return (0, default_width_info)
            }

            // Khmer coeng signs
            if next_info == khmer_coeng_eligible_letter_width_info && c == '\\u{17D2}' {
                return (-1, default_width_info)
            }

            // Buginese <a, -i> ZWJ ya ligature
            if next_info == zwj_buginese_letter_ya_width_info && c == '\\u{1A17}' {
                return (0, buginese_vowel_sign_i_zwj_letter_ya_width_info)
            }
            if next_info == buginese_vowel_sign_i_zwj_letter_ya_width_info && c == '\\u{1A15}' {
                return (0, default_width_info)
            }

            // Tifinagh bi-consonants
            if (next_info == tifinagh_consonant_width_info || next_info == zwj_tifinagh_consonant_width_info) && c == '\\u{2D7F}' {
                return (1, tifinagh_joiner_consonant_width_info)
            }
            if next_info == zwj_tifinagh_consonant_width_info && ((c >= '\\u{2D31}' && c <= '\\u{2D65}') || c == '\\u{2D6F}') {
                return (0, default_width_info)
            }
            if next_info == tifinagh_joiner_consonant_width_info && ((c >= '\\u{2D31}' && c <= '\\u{2D65}') || c == '\\u{2D6F}') {
                return (-1, default_width_info)
            }

            // Lisu tone letter combinations
            if next_info == lisu_tone_letter_mya_na_jeu_width_info && (c >= '\\u{A4F8}' && c <= '\\u{A4FB}') {
                return (0, default_width_info)
            }

            // Old Turkic ligature
            if next_info == zwj_old_turkic_letter_orkhon_i_width_info && c == '\\u{10C32}' {
                return (0, default_width_info)
            }"""

    s += f"""
            // Emoji modifier
            if next_info == emoji_modifier_width_info && is_emoji_modifier_base(c) {{
                return (0, emoji_presentation_width_info)
            }}

            // Regional indicator
            if (next_info == regional_indicator_width_info || next_info == several_regional_indicator_width_info) && (c >= '\\u{{1F1E6}}' && c <= '\\u{{1F1FF}}') {{
                return (1, several_regional_indicator_width_info)
            }}

            // ZWJ emoji
            if (next_info == emoji_presentation_width_info || next_info == several_regional_indicator_width_info || next_info == even_regional_indicator_zwj_presentation_width_info || next_info == odd_regional_indicator_zwj_presentation_width_info || next_info == emoji_modifier_width_info) && c == '\\u{{200D}}' {{
                return (0, zwj_emoji_presentation_width_info)
            }}
            if next_info == zwj_emoji_presentation_width_info && c == '\\u{{20E3}}' {{
                return (0, keycap_zwj_emoji_presentation_width_info)
            }}
            if next_info == vs16_zwj_emoji_presentation_width_info && starts_emoji_presentation_seq(c) {{
                return (0, emoji_presentation_width_info)
            }}
            if next_info == vs16_keycap_zwj_emoji_presentation_width_info && ((c >= '0' && c <= '9') || c == '#' || c == '*') {{
                return (0, emoji_presentation_width_info)
            }}
            if next_info == zwj_emoji_presentation_width_info && (c >= '\\u{{1F1E6}}' && c <= '\\u{{1F1FF}}') {{
                return (1, regional_indicator_zwj_presentation_width_info)
            }}
            if (next_info == regional_indicator_zwj_presentation_width_info || next_info == odd_regional_indicator_zwj_presentation_width_info) && (c >= '\\u{{1F1E6}}' && c <= '\\u{{1F1FF}}') {{
                return (-1, even_regional_indicator_zwj_presentation_width_info)
            }}
            if next_info == even_regional_indicator_zwj_presentation_width_info && (c >= '\\u{{1F1E6}}' && c <= '\\u{{1F1FF}}') {{
                return (3, odd_regional_indicator_zwj_presentation_width_info)
            }}
            if next_info == zwj_emoji_presentation_width_info && (c >= '\\u{{1F3FB}}' && c <= '\\u{{1F3FF}}') {{
                return (0, emoji_modifier_width_info)
            }}
            if next_info == zwj_emoji_presentation_width_info && c == '\\u{{E007F}}' {{
                return (0, tag_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0061}}' && c <= '\\u{{E007A}}') {{
                return (0, tag_a1_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_a1_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0061}}' && c <= '\\u{{E007A}}') {{
                return (0, tag_a2_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_a2_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0061}}' && c <= '\\u{{E007A}}') {{
                return (0, tag_a3_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_a3_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0061}}' && c <= '\\u{{E007A}}') {{
                return (0, tag_a4_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_a4_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0061}}' && c <= '\\u{{E007A}}') {{
                return (0, tag_a5_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_a5_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0061}}' && c <= '\\u{{E007A}}') {{
                return (0, tag_a6_end_zwj_emoji_presentation_width_info)
            }}
            if (next_info == tag_end_zwj_emoji_presentation_width_info || next_info == tag_a1_end_zwj_emoji_presentation_width_info || next_info == tag_a2_end_zwj_emoji_presentation_width_info || next_info == tag_a3_end_zwj_emoji_presentation_width_info || next_info == tag_a4_end_zwj_emoji_presentation_width_info) && (c >= '\\u{{E0030}}' && c <= '\\u{{E0039}}') {{
                return (0, tag_d1_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_d1_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0030}}' && c <= '\\u{{E0039}}') {{
                return (0, tag_d2_end_zwj_emoji_presentation_width_info)
            }}
            if next_info == tag_d2_end_zwj_emoji_presentation_width_info && (c >= '\\u{{E0030}}' && c <= '\\u{{E0039}}') {{
                return (0, tag_d3_end_zwj_emoji_presentation_width_info)
            }}
            if (next_info == tag_a3_end_zwj_emoji_presentation_width_info || next_info == tag_a4_end_zwj_emoji_presentation_width_info || next_info == tag_a5_end_zwj_emoji_presentation_width_info || next_info == tag_a6_end_zwj_emoji_presentation_width_info || next_info == tag_d3_end_zwj_emoji_presentation_width_info) && c == '\\u{{1F3F4}}' {{
                return (0, emoji_presentation_width_info)
            }}
            // Already handled above

            if next_info == kirat_rai_vowel_sign_e_width_info && c == '\\u{{16D63}}' {{
                return (0, default_width_info)
            }}
            if next_info == kirat_rai_vowel_sign_e_width_info && c == '\\u{{16D67}}' {{
                return (1, kirat_rai_vowel_sign_ai_width_info)
            }}
            if next_info == kirat_rai_vowel_sign_e_width_info && c == '\\u{{16D68}}' {{
                return (1, kirat_rai_vowel_sign_e_width_info)
            }}
            if next_info == kirat_rai_vowel_sign_e_width_info && c == '\\u{{16D69}}' {{
                return (0, default_width_info)
            }}
            if next_info == kirat_rai_vowel_sign_ai_width_info && c == '\\u{{16D63}}' {{
                return (0, default_width_info)
            }}
        }}

        let ret = lookup_width{cjk_lo}(c)
        (ret.0.reinterpret_as_int(), ret.1)
    }}
}}
...
def emit_module(
    ...
    """Outputs a MoonBit module to `out_name` using table data from `tables`.
...
///|
priv struct WidthInfo(UInt16) derive(Eq)
...
"""
        ...
        # Generate standalone constant declarations for each variant
        for variant in WidthState:
            if variant.is_carried():
                if variant.is_cjk_only():
                    module.write("// CJK only\n")
                # Convert SCREAMING_SNAKE_CASE to snake_case for let bindings
                variant_name = variant.name.lower()
                module.write(
                    f"let {variant_name}_width_info : WidthInfo = WidthInfo(0b{variant.value:016b})\n"
                )
...
pub let unicode_version : (Int, Int, Int) = {unicode_version}
""")
...
fn is_transparent_zero_width(c : Char) -> Bool {
    if lookup_width(c).0 != 0 {
        ...
        let cp = c.to_int()
        // Binary search in NON_TRANSPARENT_ZERO_WIDTHS
        let mut low = 0
        let mut high = non_transparent_zero_widths.length()
        while low < high {
            let mid = (low + high) / 2
            let (start, end) = non_transparent_zero_widths[mid]
            if cp < start {
                high = mid
            } else if cp > end {
                low = mid + 1
...
fn is_char_ligature_transparent(c : Char) -> Bool {
    match c {"""
        )
...
fn is_solidus_transparent(c : Char) -> Bool {
    ...
    is_char_ligature_transparent(c) || {
        ...
        while low < high {
            let mid = (low + high) / 2
            let (start, end) = solidus_transparent[mid]
            if cp < start {
                high = mid
            } else if cp > end {
                low = mid + 1
...
fn starts_emoji_presentation_seq(c : Char) -> Bool {
    ...
    let top_bits = cp >> 10
    let idx_of_leaf = match top_bits {"""
        )
...
fn starts_non_ideographic_text_presentation_seq(c : Char) -> Bool {
    ...
    let bottom_bits = cp & 0xFF
    // Binary search in the specific leaf array
    let ranges = match leaf_idx {"""
        )

        for msbs, i in text_presentation_idx:
            module.write(f"\n        {i} => text_presentation_leaf_{i}")
    ...
    while low < high {
        ...
        if bottom_bits < start.reinterpret_as_int() {
            high = mid
        } else if bottom_bits > end.reinterpret_as_int() {
            low = mid + 1
        } else {
            return true
        }
...
""")
        ...
        for i, table in enumerate(tables):
            ...
            if table.bytes_per_row is None:
                ...
                if table.cfged:
                    module.write("// CJK only\n")
                module.write(f"let {table.name.lower()} : Array[UInt] = [")
                ...
                module.write(
                    f"""
// CJK variant has {num_rows} rows, non-CJK has {num_primary_rows} rows
///| Autogenerated. {subtable_count} sub-table(s). Consult [`lookup_width`] for layout info.
let {table.name.lower()} : Array[Array[UInt]] = [\n"""
                )
                for row_num in range(0, num_rows):
                    ...
                    module.write("    [\n")
                    row = byte_array[
                        row_num * table.bytes_per_row : (row_num + 1)
                        * table.bytes_per_row
                    ]
...
let non_transparent_zero_widths : Array[(Int, Int)] = [
"""
        )

        for lo, hi in non_transparent_zero_widths:
            module.write(f"    (0x{lo:06X}, 0x{hi:06X}),\n")
...
///| FIXME: can we get better compression?
// CJK only
let solidus_transparent : Array[(Int, Int)] = [
"""
        )
...
let emoji_presentation_leaves : Array[Array[UInt]] = [
"""
        )
        ...
        for leaf_idx, leaf in enumerate(text_presentation_leaves):
            module.write(
                f"""
let text_presentation_leaf_{leaf_idx} : Array[(UInt, UInt)] = [
"""
            )
            for lo, hi in leaf:
        ...
        for leaf_idx, leaf in enumerate(emoji_modifier_leaves):
            module.write(
                f"""
let emoji_modifier_leaf_{leaf_idx} : Array[(UInt, UInt)] = [
"""
            )
            for lo, hi in leaf:
                module.write(f"    (0x{lo:02X}, 0x{hi:02X}),\n")
...
let normalization_test_data : Array[(String, String, String, String, String)] = [\n"""
        )
        ...
        module.write("""
///| Test that normalization test data is accessible and well-formed
test "normalization_test_data_accessibility" {
    // Test that we can access the normalization test data
    let data_length = normalization_test_data.length()
    inspect(data_length > 0, content="true")
    
    // Test that each entry has 5 components (orig, nfc, nfd, nfkc, nfkd)
    let test_count = if data_length < 10 { data_length } else { 10 }
    for i = 0; i < test_count; i = i + 1 {
        let (orig, nfc, nfd, nfkc, nfkd) = normalization_test_data[i]
        inspect(orig.length() >= 0, content="true")
        inspect(nfc.length() >= 0, content="true")
        inspect(nfd.length() >= 0, content="true")
        inspect(nfkc.length() >= 0, content="true")
        inspect(nfkd.length() >= 0, content="true")
    }
}

///| Test specific normalization cases
test "normalization_specific_cases" {
    // Test that we have some expected normalization test cases
    let mut found_decomposition = false
    let mut found_composition = false
    
    let test_count = if normalization_test_data.length() < 100 { normalization_test_data.length() } else { 100 }
    for i = 0; i < test_count; i = i + 1 {
        let (orig, nfc, nfd, _nfkc, _nfkd) = normalization_test_data[i]
        
        // Look for cases where NFC != NFD (composition/decomposition differences)
        if nfc != nfd {
            found_decomposition = true
        }
        
        // Look for cases where original != NFC (composition needed)
        if orig != nfc {
            found_composition = true
        }
    }
    
    inspect(found_decomposition, content="true")
    inspect(found_composition, content="true")
}
""")
...
def main(module_path: str):
    ...
    lookup table for character width, and write a MoonBit module utilizing that table to
    `module_filename`.

    See the generated MoonBit module for documentation of the exact width rules.
    """
